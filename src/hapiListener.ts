...
export class HapiListener extends EventEmitter{
  listen(_port: any, maybeCallback?: any, maybeCallback2?: any) {
    let callback: Callback | undefined = typeof maybeCallback === 'function' ? maybeCallback : undefined
    if (!callback) {
  ...
  get handler() {
    return (req: IncomingMessage, res: ServerResponse) => this.emit('request', req, res)
  }
}
